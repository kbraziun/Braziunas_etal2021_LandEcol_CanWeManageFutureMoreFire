...
/* custom start and stop outputs

function onYearBegin() {
	if ([10,55,100].indexOf(Globals.year) != -1) {
		Globals.startOutput("sapling");
		Globals.startOutput("saplingdetail");
		Globals.startOutput("carbon");
		Globals.startOutput("dynamicstand");
	}

	if ([31,76].indexOf(Globals.year) != -1) {
		Globals.stopOutput("sapling");
		Globals.stopOutput("saplingdetail");
		Globals.stopOutput("carbon");
